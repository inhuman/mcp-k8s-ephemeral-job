...
// Register adds the run_job tool to the server. The in/out schema is derived from the Input/Output types.
...
// Execute runs run_job end to end: validate, resolve limits, spawn, collect, Output.
// An error is returned only for invalid input or an executor failure; an unsuccessful
// run (status=failed/timeout/error) is a valid Output carrying that status.
func (t *Tool) Execute(ctx context.Context, in Input) (Output, error) {
	...
	resources, err := ResolveResources(in.Limits, t.opts.DefaultCPU, t.opts.DefaultMemory)
	if err != nil {
		return Output{}, fmt.Errorf("invalid arguments: %w", err)
	}
	...
	out := toOutput(res)
	// Caller data is never logged — metadata only.
