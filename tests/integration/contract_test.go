...
// Контракт create/delete на РЕАЛЬНОМ kube-apiserver (Принцип V — без моков k8s API).
// Запуск: envtest/CI предоставляет endpoint через KUBECONFIG (или in-cluster).
//
...
func TestContractCreateDelete(t *testing.T) {
	...
	job, err := manifest.Build(manifest.Params{
		...
		Image: "busybox:1.36", Command: []string{"true"}, CPU: "100m", Memory: "64Mi",
