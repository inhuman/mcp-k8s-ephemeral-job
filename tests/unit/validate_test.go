...
func TestImageResolver(t *testing.T) {
	r := runjob.NewImageResolver([]string{
		"registry.example.com/library/busybox:1.36",
		"registry.example.com/library/python:3.13-slim",
	})
	cases := map[string]string{
		"busybox":                   "registry.example.com/library/busybox:1.36",
		"busybox:latest":            "registry.example.com/library/busybox:1.36",
		"busybox:1.35":              "registry.example.com/library/busybox:1.36", // tag ignored
		"docker.io/library/busybox": "registry.example.com/library/busybox:1.36",
		"python":                    "registry.example.com/library/python:3.13-slim",
		"python:3.11-slim":          "registry.example.com/library/python:3.13-slim",
		"BusyBox":                   "registry.example.com/library/busybox:1.36", // case-insensitive
	}
...
func TestImageResolverMultipleVersions(t *testing.T) {
	r := runjob.NewImageResolver([]string{
		"registry.example.com/library/busybox:1.36",
		"registry.example.com/library/busybox:1.35",
	})
	// Bare base name → the FIRST listed (default), not silently shadowed.
	if got, _ := r.Resolve("busybox"); got != "registry.example.com/library/busybox:1.36" {
		t.Errorf("Resolve(busybox) = %q, want :1.36 default", got)
	}
	// Exact version → that exact one.
	if got, _ := r.Resolve("busybox:1.35"); got != "registry.example.com/library/busybox:1.35" {
		t.Errorf("Resolve(busybox:1.35) = %q, want :1.35", got)
	}
	if got, _ := r.Resolve("busybox:1.36"); got != "registry.example.com/library/busybox:1.36" {
		t.Errorf("Resolve(busybox:1.36) = %q, want :1.36", got)
	}
	// Unknown version → fall back to the base default, still runs.
	if got, ok := r.Resolve("busybox:9.9"); !ok || got != "registry.example.com/library/busybox:1.36" {
